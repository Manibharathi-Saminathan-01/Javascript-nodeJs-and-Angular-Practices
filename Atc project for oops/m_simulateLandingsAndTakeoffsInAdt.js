...
class Airport {
  ...
  simulateLandingsAndTakeoffsPeriodically() {
    ...
    this.printPeriodically();
    this.secondPhaseOfNoEmergencyLandings();
    this.printPeriodically();
  }
...
class Atc extends Airport {
  constructor(inputCard) {
    super();
    this.inputCard = inputCard;
  }
  getDataFromTerminal() {
    fs.readFile(this.inputCard, "utf-8", (error, data) => {
      if (error) throw error;
      const inputdata = data.split("\n");
      // Here we are calling our initial methods
      console.log("");
      console.log(
        `---------------------- ✈️ ${this.inputCard.slice(
          0,
          this.inputCard.length - 4
        )} ✈️---------------------------`
      );

      this.convertingTextFileIntoTerminalCard(inputdata);
      this.insertingDatasIntoTheAllQueues();
      this.simulateLandingsAndTakeoffsPeriodically();
    });
  }
}
// objects for Atc derived class
const kempagowdaAirportAtc = new Atc("kempaGowdaTerminal.txt");
const chennaiAirportAtc = new Atc("chennaiAirportTerminal.txt");
const hydrabadAirportAtc = new Atc("hydrabadAirportTerminal.txt");
kempagowdaAirportAtc.getDataFromTerminal();
chennaiAirportAtc.getDataFromTerminal();
hydrabadAirportAtc.getDataFromTerminal();
